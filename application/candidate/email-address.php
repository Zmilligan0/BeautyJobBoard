...
if (isset($_POST['submit'])) {
    ...
    if($email != ""){
        $email = filter_var($email, FILTER_SANITIZE_EMAIL); // Cleanse this var of any malicious strings
        if(!filter_var($email, FILTER_VALIDATE_EMAIL )){
          $valid = 0;  
          $emailMsg = "<p>Please enter a correctly formatted email address.</p>";
        }
    }else{
        $valid = 0;  
        $emailMsg = "<p>Please enter an email address.</p>";
    }

    if($valid ==1){

        $msgSuccess = "Successfully updated the email address.";
        $sql = "UPDATE user SET 
        email = '$email'
        ...
        <div class="container mt-5">
            <div class="card main-card mx-auto d-block ">
                ...
                <div class="card-body">
                    ...
                    <p class="p-2">
                        <!-- <a class="text-decoration-none text-muted" href="login-security"> -->
                    ...
                    <form class="mt-5" id="myform" name="myform" method="post" action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>">
                        ...
                        <div class="mt-3 col-lg-11">
					        ...
					        <?php if($emailMsg){echo $msgPreError.$emailMsg.$msgPost;} ?>
                            <div id="emailHelp" class="form-text mt-3">A confirmation will be sent to this account.
                                Click on the confirmation link to verify and add this email.</div>
                        </div>
                        <button type="submit" name="submit" class="btn btn-danger mt-3">Update email address</button>
                    </form>
                </div>
                <!-- Account Access Card Body -->
            </div>
