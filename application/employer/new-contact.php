...
// get the employer ID
include 'includes/contact-post.php';
...
if (isset($_GET['contact_id'])) {
  ...
  $employer_id = $_SESSION['employer_id'];
  $get_one_sql = "SELECT contact_id,employer_id,first_name,last_name,email,notes
    FROM contact WHERE contact_id = $contact_id AND employer_id = $employer_id";
  $get_one_result = $jobConn->query($get_one_sql);
  if ($get_one_result->num_rows == 1) {
    $get_one_row = $get_one_result->fetch_assoc();
    $first_name = $get_one_row['first_name'];
    $last_name = $get_one_row['last_name'];
    $position = $get_one_row['notes'];
    $email = $get_one_row['email'];
  } else {
    $message = "<p>There was a problem retrieving the inforamtion</p>";
    ...
    <div class="d-flex">
      ...
      <a href="edit-job.php" class="nav-link text-white">
        ...
        Edit Job Post
          ...
          <li>
            <a href="new-contact?id=<?php echo $id ?>" class="nav-link text-white bg-color">
      ...
      <div class=" p-3 container p-5 pt-1">
        <h1 class="text-decoration-underline">Contact</h1>
